...
public:
    ...
    NOALIAS constexpr Quaternion<T> operator*(const Quaternion<T> q) const noexcept
    {
    ...
    NOALIAS constexpr Vec3<T> operator*(const Vec3<T> p) const noexcept
    {
    ...
    NOALIAS constexpr friend Vec3<T> operator*(const Vec3<T> p, const Quaternion<T> q) noexcept
    {
        return q * p;
    }
     ...
     */
    template<typename U>
    [[nodiscard]] Quaternion<U> Normalized() const
    {
    ...
    [[nodiscard]] NOALIAS constexpr static Quaternion<U> AngleAxis(const Radian angle, const Vec3<U> axis)
    {
        ...
        q.W = MathUtility::Cos(angle / 2.f);
        q.V = Vec3<U>::Normalized(axis) * MathUtility::Sin(angle / 2.f);
    ...
    {
        float cosX = MathUtility::Cos(x * 0.5f);
        float sinX = MathUtility::Sin(x * 0.5f);
        float cosY = MathUtility::Cos(y * 0.5f);
        float sinY = MathUtility::Sin(y * 0.5f);
        float cosZ = MathUtility::Cos(z * 0.5f);
        float sinZ = MathUtility::Sin(z * 0.5f);
