...
// Constantin, Olivier, Remy, Alexis
#include "Angle.h"
...
public:
    ...
    Vec4(Vec4<T>&& vec) noexcept = default;
    Vec4<T>& operator=(const Vec4<T>& vec) = default;
    ~Vec4() = default;
    ...
    inline const static Vec4<T> Zero = Vec4<T>(0, 0, 0, 0);
    inline const static Vec4<T> One = Vec4<T>(1, 1, 1, 1);
    inline const static Vec4<T> Up = Vec4<T>(0, 1, 0, 0);
    inline const static Vec4<T> Down = Vec4<T>(0, -1, 0, 0);
    inline const static Vec4<T> Left = Vec4<T>(-1, 0, 0, 0);
    inline const static Vec4<T> Right = Vec4<T>(1, 0, 0, 0);
    inline const static Vec4<T> Forward = Vec4<T>(0, 0, 1, 0);
    inline const static Vec4<T> Backward = Vec4<T>(0, 0, -1, 0);

#pragma region Operators

    constexpr Vec4<T> operator+(const Vec4<T> vec) const noexcept
	{
    ...
    constexpr Vec4<T> operator*(const T scalar) const noexcept
	{
		return Vec4<T>(X * scalar, Y * scalar, Z * scalar, W * scalar);
	}

    constexpr Vec4<T> operator/(const T scalar) const
	{
		if (scalar == 0)
		{
    ...
    constexpr friend Vec4<T> operator*(const T scalar, const Vec4<T> vec) noexcept
	{
		return vec * scalar;
	}
...
#pragma endregion
    ...
    constexpr static T Dot(const Vec4<T> vecA, const Vec4<T> vecB) noexcept
	{
    ...
    template <typename U = T>
    [[nodiscard]] constexpr static Vec4<U> Lerp(const Vec4<T> vecA, const Vec4<T> vecB, float t) noexcept
	{
    ...
    template <typename U = T>
    [[nodiscard]] constexpr Vec4<U> Lerp(const Vec4<T> vec, float t) const noexcept
	{
		return Lerp(*this, vec, t);
	}

    [[nodiscard]] static float Distance(const Vec4<T> vecA, const Vec4<T> vecB) noexcept
	{
		return (vecA - vecB).Length();
	}

    [[nodiscard]] float Distance(const Vec4<T> vec) const noexcept
	{
